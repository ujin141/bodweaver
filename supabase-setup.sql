-- ============================================================
-- BODWEAVER — Supabase 완전 스키마 (Production Clean)
-- 모의 데이터(INSERT) 없는 프로덕션 버전
...
CREATE TABLE IF NOT EXISTS public.users (
    ...
    avatar_emoji TEXT DEFAULT '🎮',               -- 이모지 아바타
    ...
    rating_avg  DECIMAL(3,2) DEFAULT 5.00,        -- 평균 평점
    match_count INT DEFAULT 0,                    -- 총 매칭 횟수
    latitude    DECIMAL(10, 8),                   -- 유저 위치 위도
    longitude   DECIMAL(11, 8),                   -- 유저 위치 경도
...
CREATE TABLE IF NOT EXISTS public.rooms (
    ...
    latitude     DECIMAL(10, 8),                  -- 모임 장소 위도
    longitude    DECIMAL(11, 8),                  -- 모임 장소 경도
    created_at   TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.messages (
    ...
    text        TEXT NOT NULL,
    is_system   BOOLEAN DEFAULT FALSE,            -- 시스템 메시지 (입장/퇴장)
    created_at  TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.notifications (
    ...
    user_id    UUID REFERENCES public.users(id) ON DELETE CASCADE,
    type       TEXT NOT NULL,
    -- 타입: join / match / chat / review / system / plus
    title      TEXT NOT NULL,
    body       TEXT,
...
-- plus_subscriptions
DROP POLICY IF EXISTS "plus_select_self"  ON public.plus_subscriptions;
CREATE POLICY "plus_select_self"  ON public.plus_subscriptions FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "plus_insert_self"  ON public.plus_subscriptions;
...
-- ============================================================
-- ✅ 완료! 모든 테이블, RLS, 함수가 생성되었습니다.
-- 이 파일은 중복 실행해도 에러 없이 안전합니다.
-- ============================================================


-- ============================================================
-- 🚨 [필요 시 수동 실행] DB 모든 모임/채팅 데이터 초기화
-- 필요한 줄만 선택해서 실행하세요. (users 테이블은 삭제 안 됨)
-- ============================================================
